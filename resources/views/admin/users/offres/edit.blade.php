...
<div class="container">
    ...
    <div class="row">
        ...
        <div class="col-md-12">
          <div class="tile">
            <h3 class="tile-title mb-4">Enregistrer une Promo</h3>
            <div class="tile-body">
            <form method="POST" action="{{route('admin.offres.update',$offre->id)}}" enctype="multipart/form-data">
                @csrf
                    @method('PATCH')
                <div class="form-group row {{$errors->has('title') ? 'has-error' : ''}}">
                  <label class="control-label col-md-3">Titre</label>
                  <div class="col-md-8">
                    <input class="form-control" name="title" value="{{ old('title') ?? $offre->title }}" type="text">
                    @error('title')
                ...
                <div class="form-group row {{$errors->has('description') ? 'has-error' : ''}}">
                  ...
                  <div class="col-md-8">
                    <textarea class="form-control" name="description"  rows="4">{{ $offre->description }}</textarea>
                    @error('description')
                    <div class="alert alert-danger">{{ $message }}</div>
                    @enderror
                  </div>
                ...
                <div class="form-group row {{$errors->has('image') ? 'has-error' : ''}}">
                  ...
                  <div class="col-md-8">
                    <input class="form-control" name="image" value="{{ old('image') ?? $offre->image }}" type="file">
                    @error('file')
                    <div class="alert alert-danger">{{ $message }}</div>
                    @enderror
                  </div>
                ...
                <div class="form-group row {{$errors->has('date_debut') ? 'has-error' : ''}}">
                    <label class="control-label col-md-3">Date de debut</label>
                      <div class="col-md-8">
                          <input class="form-control" name="date_debut" id="demoDate" value="{{ old('date_debut') ?? $offre->date_debut }}" type="date">
                          @error('date_debut')
                          <div class="alert alert-danger">{{ $message }}</div>
                          @enderror
                      </div>
                ...
                <div class="form-group row {{$errors->has('date_fin') ? 'has-error' : ''}}" >
                    <label class="control-label col-md-3">Date de fin</label>
                      <div class="col-md-8">
                          <input class="form-control" name="date_fin" id="demoDate" value="{{ old('date_fin') ?? $offre->date_fin }}" type="date">
                          @error('date_fin')
                          <div class="alert alert-danger">{{ $message }}</div>
                          @enderror
                      </div>
